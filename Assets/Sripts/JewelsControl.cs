...
public class JewelsControl : MonoBehaviour
{
    ...
    private float xSlip = 0f;
    private float ySlip = -5f;
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            ...
            for (int j = 0; j < 9; j++)
            {
                ...
                field[i][j].transform.position = new Vector3(i * 2f - 8f + xSlip, j * 2f - 8f + ySlip, 0f);
            }
        }
    }
        ...
        foreach (Touch touch in Input.touches)
        {
            ...
            else if (touch.phase == TouchPhase.Ended)
            {
                ...
                for (int i = 0; i < field.Count; i++)
                {
                    ...
                    {
                        if (Mathf.Abs(field[i][j].transform.position.x - pos.x) <= 1f && Mathf.Abs(field[i][j].transform.position.y - pos.y) <= 1f)
